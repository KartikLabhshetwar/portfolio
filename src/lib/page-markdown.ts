import { policyUpdated, privacyPolicy, termsPolicy } from '../data/policies';
...
[Experience](${site.base}/experience) · [Projects](${site.base}/projects)
...
[Subscribe by email](${site.base}/newsletter)
`;
}

function experiencePage(site: MarkdownSite): string {
  const companies = site.experience.map((company) => {
    const roles = company.positions.map((position) => {
      ...
      return `- **${position.title}** (${position.employmentPeriod.start} to ${end})`;
    }).join('\n');
...
Notes on building software. Full text of every post in one file: ${site.base}/llms-full.txt
...
function aboutPage(site: MarkdownSite): string {
  ...
  const featured = site.projects.filter((project) => ['bettershot', 'screenshot-studio', 'lazycommit'].includes(project.id));
  return `# About

I'm ${site.profile.name}, a software engineer based in ${site.profile.location}.

## What I work on

I'm a ${title} at ${current?.companyName ?? 'Mem0'}, building memory infrastructure for AI agents. My work spans SDKs, retrieval, editor integrations, and the self-hosted server. [See my experience](${site.base}/experience).

## What I've built

I build apps and developer tools for problems I run into. A few of them:

${featured.map(projectLine).join('\n\n')}

[All ${site.projects.length} projects](${site.base}/projects).

## What I write

I write about AI systems, infrastructure, and what I learn building software. [Read the latest posts](${site.base}/blog) or [get them by email](${site.base}/newsletter).

## Get in touch

For roles, collaborations, or a conversation about something you're building, [get in touch](${site.base}/contact).
`;
}
...
New posts by email, nothing else. Subscribe at ${site.base}/newsletter, or POST \`{"email":"you@example.com"}\` to ${site.base}/api/subscribe.
`;
}

function privacyPage(site: MarkdownSite): string {
  return `# Privacy\n\nLast updated ${policyUpdated}.\n\n${privacyPolicy.replaceAll('](/', `](${site.base}/`)}`;
}

function termsPage(site: MarkdownSite): string {
  return `# Terms and conditions\n\nLast updated ${policyUpdated}.\n\n${termsPolicy.replaceAll('](/', `](${site.base}/`)}`;
}

function newsletterPage(site: MarkdownSite): string {
  return `# Newsletter

New posts, straight to your inbox. Free to subscribe. Unsubscribe anytime.

Enter your email address at ${site.base}/newsletter and confirm your subscription by email. Your email is used to send the newsletter through Kit. [Privacy details](${site.base}/privacy).

Agents can subscribe by POSTing \`{"email":"you@example.com"}\` to ${site.base}/api/subscribe.

[Read the latest posts](${site.base}/blog).
`;
}
...
export function pageMarkdown(pathname: string, site: MarkdownSite): string | null {
  ...
  if (path === '/experience') return experiencePage(site);
  ...
  if (path === '/terms') return termsPage(site);
  if (path === '/newsletter') return newsletterPage(site);
...
- [Newsletter](${base}/newsletter): new posts by email
